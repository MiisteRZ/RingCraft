...
public class ItemAnimationFactory {
	...
	public static void animatedItems(PlayerTickEvent.Post event) {
		...
		if (mainhandItem.getItem() instanceof GeoItem || offhandItem.getItem() instanceof GeoItem) {
			if (mainhandItem.getItem() instanceof BlazeSoulItemItem animatable) {
				...
				if (!animation.isEmpty()) {
					...
					if (event.getEntity().level().isClientSide()) {
						((BlazeSoulItemItem) event.getEntity().getMainHandItem().getItem()).animationprocedure = animation;
					}
				}
			}
			if (offhandItem.getItem() instanceof BlazeSoulItemItem animatable) {
				...
				if (!animation.isEmpty()) {
					...
					if (event.getEntity().level().isClientSide()) {
						((BlazeSoulItemItem) event.getEntity().getOffhandItem().getItem()).animationprocedure = animation;
					}
				}
			}
		}
